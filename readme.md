...
A second source of inspiration is [dependency grammar](https://en.wikipedia.org/wiki/Dependency_grammar), a flexible framework used in Computational Linguistics research for a wide variety of natural language processing tasks, such as algorithms that translate one language into another. Inspired by ideas from natural language processing, Cube uses a form of dependency grammar that is easily extensible. This simplifies the creation of domain specific languages, as well as [polyglot programs], which combine features of different programming languages (such as functional programming and SQL) to solve problems more easily that using a single language.
