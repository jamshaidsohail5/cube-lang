...
In Cube, both functions and lambdas are strongly typed and can be assigned to variables and passed to other functions as arguments. Cube uses arrow syntax for lambda expressions, such as `string -> int` for a function that maps strings to integers. In the following example, `transform` is a higher-order function that accepts a `dataset -> boolean` parameter.
...
In longer functions, the `output` keyword can be used to return an output value. The example higher-order function below accepts a block of code as input, then executes the lambda to output a benchmark measurement.
...
Cube supports class-based object-oriented programming. Similar to [C#](https://en.wikipedia.org/wiki/C_Sharp_(programming_language)), objects in cube can have auto-generated properties with getters and setters.

A simple example of a class is a temperature calculator with a Celsius value that can be increased, or converted to Fahrenheit. The `define` keyword is used for class definitions. 
...
Cube supports interfaces, polymorphism and abstract types.
...
Parallel computing using low-level threads is challenging to implement and difficult to test. To simplify concurrent and distributed programming, Cube provides an intuitive agent-based framework for message and stream processing.
