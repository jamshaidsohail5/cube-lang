...
What makes pseudocode so powerful, is that any computer program, even a highly complex one, can be described using combination of simple mathematical notation, and basic operations with intuitive names (such as 'if', 'while', 'do' and 'end'). At a high-level, programs written in pseudocode style avoid unnecessary punctuation and unclear symbols, and prefer to encode the meaning of operations using conventional words (lexical semantics).
